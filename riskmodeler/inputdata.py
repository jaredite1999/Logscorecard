...
class inputdata():
    ...
    def newdatainput(self):
        ...
        page = ttk.Frame(self.master, padding=(24, 20))
        page.grid(column=0, row=0, sticky='nsew')
        page.grid_columnconfigure(0, weight=1)
        ttk.Label(page, text='导入建模数据', style='SectionTitle.TLabel').grid(column=0, row=0, sticky='w')
        ttk.Label(page, text='选择源数据、设置名称与角色，随后进入变量配置。', style='Hint.TLabel').grid(column=0, row=1, sticky='w', pady=(4, 16))
        form = ttk.LabelFrame(page, text='数据源设置', style='Card.TLabelframe', padding=(18, 16))
        form.grid(column=0, row=2, sticky='ew')
        configure_form_grid(form, 2)

        def selectExcelfile():
            sfname = filedialog.askopenfilename(title='选择CSV文件', filetypes=[('CSV', '*.csv')])
            clear_and_fill(self.E1, sfname)
            entry_name = os.path.basename(sfname).replace('.csv','')
            clear_and_fill(self.E2, entry_name)

        ttk.Label(form, text="数据集路径（CSV）", style='Field.TLabel').grid(column=0, row=0, sticky='w')
        path_row = ttk.Frame(form)
        path_row.grid(column=1, row=0, sticky='ew', pady=(0, 10))
        path_row.grid_columnconfigure(0, weight=1)
        self.E1 = ttk.Entry(path_row, width=36)
        self.E1.grid(column=0, row=0, sticky='ew')
        button1 = ttk.Button(path_row, text='浏览', style='Secondary.TButton', command=selectExcelfile)
        button1.grid(column=1, row=0, padx=(10, 0))

        ttk.Label(form, text="数据集名称", style='Field.TLabel').grid(column=0, row=1, sticky='w')
        self.E2 = ttk.Entry(form, width=26)
        self.E2.grid(column=1, row=1, sticky='ew', pady=(0, 10))

        ttk.Label(form, text="数据集编码", style='Field.TLabel').grid(column=0, row=2, sticky='w')
        self.E3 = ttk.Combobox(form, state='readonly')
        self.E3["value"] = ['utf-8', 'gbk']
        self.E3.current(0)
        self.E3.grid(column=1, row=2, sticky='ew', pady=(0, 10))

        ttk.Label(form, text="数据集角色", style='Field.TLabel').grid(column=0, row=3, sticky='w')
        self.role = ttk.Combobox(form, state='readonly')
        self.role["value"] = ['Training model', 'Reject', 'out of time sample','Score']
        self.role.current(0)
        self.role.grid(column=1, row=3, sticky='ew', pady=(0, 10))

        ttk.Label(form, text='导入后会进入变量角色设置，可继续预览、探索和保存数据集。', style='Hint.TLabel').grid(column=1, row=4, sticky='w')
        test_button4 = ttk.Button(form, text='读取并继续', style='Accent.TButton')
        test_button4.grid(column=1, row=5, sticky='w', pady=(18, 0))
