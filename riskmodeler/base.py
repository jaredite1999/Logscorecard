...
class group_func():
    ...
    def numericvar_apior(self, inputdata, col, s_bin_num, target, specialcode_list):
        ...
        if len(inputdata[inputdata[col].isnull()]) > 0:
            ...
            tempg = pd.DataFrame({'s_group': -1, 'value': 'miss', 'miss_s': True},
                                 index=[0])
            s_group_data = pd.concat([s_group_data, miss_data])
            group_info_base = pd.concat([group_info_base, tempg])
        if specialcode_list != []:
            i = -2
            for special_value in specialcode_list:
                ...
                s_group_data = pd.concat([s_group_data, temp])
                group_info_base = pd.concat([group_info_base,temps])
                i = i - 1
    ...
    def numericvar(self, inputdata, col, s_bin_num, target, criterion, splitter, max_depth, min_samples_leaf,
        ...
        if len(inputdata[inputdata[col].isnull()]) > 0:
            ...
            tempg = pd.DataFrame({'s_group': -1, 'f_group': -1, 'value': 'miss', 'miss_s': True, 'miss_f': True},
                                 index=[0])
            f_group_data = pd.concat([f_group_data, miss_data])
            group_info_base = pd.concat([group_info_base, tempg])
        if specialcode_list != []:
            i = -2
            for special_value in specialcode_list:
    ...
    def charvar(self, inputdata, col, min_num, min_pct, target, criterion, splitter, max_depth, min_samples_leaf,
        ...
        f_group_data = pd.concat([f_group_data, miss_part])
        tt1 = f_group_data.groupby(['f_group']).agg(
    ...
    def numericexist(self, inputdata, col, group_info_old, target, modify=False, add_value=0, data_only=False):
        ...
        if data_only == True:
            #f_group_data.to_csv('g:\\f.csv',encoding='gbk')
            #woe_data.to_csv('g:\\w.csv', encoding='gbk')
