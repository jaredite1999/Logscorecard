...
from .ui import apply_app_style, clear_and_fill, configure_form_grid, set_window_ready
...
class scorecard():
    ...
    def __init__(self):
        ...
        self.project_seting = {}
        self.project_detail = empty_project_detail()
        self.root = Tk()
        self.style = apply_app_style(self.root)
        self.status_var = tk.StringVar(value='准备开始新的评分卡项目')
    ...
    def Start_UI(self):
        ...
        set_window_ready(self.start_window_base, '风险评分卡项目', 760, 420)
        self.start_window_base.grid_columnconfigure(0, weight=1)
        page = ttk.Frame(self.start_window_base, style='App.TFrame', padding=(28, 24))
        page.grid(column=0, row=0, sticky='nsew')
        page.grid_columnconfigure(0, weight=1)
        ttk.Label(page, text='风险评分卡工作台', style='Title.TLabel').grid(column=0, row=0, sticky='w')
        ttk.Label(page, text='创建新项目或导入既有项目，继续你的建模流程。', style='Subtitle.TLabel').grid(column=0, row=1, sticky='w', pady=(6, 18))
        ...
        self.start_window = ttk.LabelFrame(cards, text='创建新项目', style='Card.TLabelframe', padding=(18, 16))
        name = tk.StringVar(value='scorecard1')

        def selectExcelfold():
            sfname = filedialog.askdirectory()
            clear_and_fill(self.project_path_E, sfname)

        configure_form_grid(self.start_window, 2)
        ttk.Label(self.start_window, text="项目路径", style='Field.TLabel').grid(column=0, row=0, sticky='w')
        path_row = ttk.Frame(self.start_window, style='Card.TFrame')
        path_row.grid(column=1, row=0, sticky='ew', pady=(0, 10))
        path_row.grid_columnconfigure(0, weight=1)
        self.project_path_E = ttk.Entry(path_row, width=36)
        self.project_path_E.grid(column=0, row=0, sticky='ew')
        button1 = ttk.Button(path_row, text='浏览', style='Secondary.TButton', command=selectExcelfold)
        button1.grid(column=1, row=0, padx=(10, 0))

        ttk.Label(self.start_window, text="项目名称", style='Field.TLabel').grid(column=0, row=1, sticky='w')
        self.project_name_E = ttk.Entry(self.start_window, textvariable=name, width=24)
        self.project_name_E.grid(column=1, row=1, sticky='ew', pady=(0, 10))
        ttk.Label(self.start_window, text='建议使用英文、数字或下划线，便于后续保存节点文件。', style='Hint.TLabel').grid(column=1, row=2, sticky='w')

        test_button4 = ttk.Button(self.start_window, text='创建项目', style='Accent.TButton')
        test_button4.grid(column=1, row=3, sticky='w', pady=(18, 0))
        test_button4.bind("<Button-1>", self.new_project)
        self.start_window.grid(column=0, row=0, sticky='nsew', padx=(0, 12))

        def selectExcelfile():
            sfname = filedialog.askopenfilename(title='选择project文件', filetypes=[('project', '*.project')])
            clear_and_fill(self.project_path_Ex, sfname)

        self.start_window_ex = ttk.LabelFrame(cards, text='导入现有项目', style='Card.TLabelframe', padding=(18, 16))
        configure_form_grid(self.start_window_ex, 2)

        ttk.Label(self.start_window_ex, text="项目文件", style='Field.TLabel').grid(column=0, row=0, sticky='w')
        import_row = ttk.Frame(self.start_window_ex, style='Card.TFrame')
        import_row.grid(column=1, row=0, sticky='ew', pady=(0, 10))
        import_row.grid_columnconfigure(0, weight=1)
        self.project_path_Ex = ttk.Entry(import_row, width=36)
        self.project_path_Ex.grid(column=0, row=0, sticky='ew')
        button1 = ttk.Button(import_row, text='浏览', style='Secondary.TButton', command=selectExcelfile)
        button1.grid(column=1, row=0, padx=(10, 0))
        ttk.Label(self.start_window_ex, text='选择之前保存的 `.project` 文件，恢复建模上下文。', style='Hint.TLabel').grid(column=1, row=1, sticky='w')

        test_button5 = ttk.Button(self.start_window_ex, text='导入项目', style='Accent.TButton')
        test_button5.grid(column=1, row=2, sticky='w', pady=(18, 0))
        test_button5.bind("<Button-1>", self.load_project)
        self.start_window_ex.grid(column=1, row=0, sticky='nsew')

        ttk.Label(page, textvariable=self.status_var, style='Status.TLabel').grid(column=0, row=3, sticky='w', pady=(16, 0))

    def load_project(self, event):
        try:
            project_add = self.project_path_Ex.get()
            self.project_detail = load_project(project_add)
            self.project_path = project_add
            self.project_name = self.project_detail[self.project_detail['模块类型'] == 'project']['模块名字'][0]
            self.project_detail = mark_project_path(self.project_detail, self.project_path)
            self.status_var.set('项目已加载，正在进入工作台')
    ...
    def new_project(self, event):
        ...
        if exists(self.project_path)==False:
            ...
            self.project_detail = append_or_replace_node(
                self.project_detail,
                build_project_record(self.project_name, self.project_path),
            )

            try:
                self.save_project()
                self.status_var.set('项目创建成功，正在打开工作台')
            except Exception as e:
                tk.messagebox.showwarning('错误', e)
                self.start_window_base.destroy()
                self.root.destroy()
    ...
    def base_UI(self):
        ...
        self.root.update()
        self.root.deiconify()
        set_window_ready(self.root, self.project_name, 1180, 760, resizable=True)
    ...
    def save_project(self):
        save_project(self.project_detail, self.project_path)

    def _update_project_detail_from_node(self, new_node, old_node_name=None):
        node_setting = getattr(new_node, 'node_setting', None)
        if not node_setting:
            return
        record = build_node_record(node_setting)
        updated = append_or_replace_node(self.project_detail, record)
        if old_node_name and old_node_name != record.node_name:
            updated = updated[updated['模块名字'] != old_node_name]
        self.project_detail = updated.reset_index(drop=True)

    def _load_node_info(self, node_save_path):
        return load_pickle_file(node_save_path)
    ...
    def func_menu(self, func):
        try:
            ...
            if new_node.save != 'Y':
                ...
                try:
                    self._update_project_detail_from_node(new_node)
                    # del new_node
                    self.refresh_df(self.root, self.project_detail)
                except Exception as e:
    ...
    def setting(self, rowclicked, colclicked):
        ...
        try:
            node_info = self._load_node_info(node_save_path)
            flag_error = 0
        except Exception as e:
            flag_error = 1
            tk.messagebox.showwarning('错误', e)
        if flag_error != 1:
            try:
                ...
                self.root.wait_window(self.root2)
                try:
                    self._update_project_detail_from_node(new_node, old_node_name=node_name)
                    self.refresh_df(self.root, self.project_detail)
                except:
                    pass
                # except Exception as e:
